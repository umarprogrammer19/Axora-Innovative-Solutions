      ...
      <Container>
        <Reveal className="max-w-[46ch]">
          <h2 className="text-[clamp(1.75rem,3.4vw,2.75rem)] font-semibold leading-[1.12] text-onlight">
            Recent{" "}
            <span className="bg-gradient-to-r from-azure to-onlight bg-clip-text text-transparent">
              work
            </span>
          </h2>
              ...
              <dl className="mt-8 grid grid-cols-2 gap-6 border-t border-paper-line pt-7">
                {lead.metrics.map((m) => (
                  <div key={m.label}>
                    ...
                    <dd>
                      <span className="block font-mono text-[1.75rem] leading-none tracking-tight text-azure">
                        {m.value}
                      </span>
                ...
                <dl className="mt-6 flex flex-wrap gap-x-10 gap-y-4">
                  {project.metrics.map((m) => (
                    <div key={m.label}>
                      <dt className="sr-only">{m.label}</dt>
                      <dd>
                        <span className="block font-mono text-[1.25rem] leading-none tracking-tight text-azure">
                          {m.value}
                        </span>
